...
from lib.util import systool
from lib.util.handler import ExceptionHandler
import inspect
import shutil
import os
import glob
...
#============
__LIST_LOGNAMES = {'runall':'runall_{}.log'}
__LOGNAME = 'latest.log'
#===========
...
def log_message(message, verbosity_level):
  ...
  global __LOGDIR
  if (__LOGDIR == None):
    __LOGDIR = systool.maindir_realpath() + 'logs' + os.sep

  systool.makedirs(__LOGDIR)
  logfile_path = __LOGDIR + __LOGNAME

  if (systool.file_exists(logfile_path)):
    if (systool.file_size(logfile_path) > __MAX_LOG_SIZE):
      destfile = __LOGDIR + ('%s.log' % systool.now_string(systool.ISO_DATETIME_FORMAT_FSAFE))
...
def debug(message):
  ...
  os = '[%s%sDEBUG] %s' % (systool.now_string(), __caller_string(), message)
  ls = os
...
def info(message):
  ...
  os = '[%s%sINFO] %s' % (systool.now_string(), __caller_string(), message)
  ls = os
...
def success(message):
  ...
  os = '%s[%s%sSUCCESS] %s%s' % (sc, systool.now_string(), __caller_string(), message, ec)
  ls = '[%s%sSUCCESS] %s' % (systool.now_string(), __caller_string(), message)
...
def warning(message):
  ...
  os = '%s[%s%sWARNING] %s%s' % (sc, systool.now_string(), __caller_string(), message, ec)
  ls = '[%s%sWARNING] %s' % (systool.now_string(), __caller_string(), message)
...
def error(message):
  ...
  os = '%s[%s%sERROR] %s%s' % (sc, systool.now_string(), __caller_string(), message, ec)
  ls = '[%s%sERROR] %s' % (systool.now_string(), __caller_string(), message)
...
def fail(message):
  ...
  os = '%s[%s%sFAIL] %s%s' % (sc, systool.now_string(), __caller_string(), message, ec)
  ls = '[%s%sFAIL] %s' % (systool.now_string(), __caller_string(), message)
